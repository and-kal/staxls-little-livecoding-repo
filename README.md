# Staxl livecoding performance material

these are the files I use for my livecoding performances

- `0-Boilerplate` - boilerplate Ableton project with all the MIDI channels and CC mappings set up
- `superdirt-startup-files` - some variants of the Supercollider/SuperDirt x TidalCycles base setup with some optional extra units (like limiter, reverb etc.)
- `0-boilerplate.tidal` - TC code goes here
- `osc-to-midi.py` - when working with MIDI only, Supercollider isn't necessary and TidalCycles OSC can be send to your MIDI Client directly, using this script

the code for my performances and recordings lives in the different branches
