...
XiiLangInstr {
	...
	*new {| project="default", loadsamples=true, numChannels |
	...
	makePan2SynthDefs {| argproject, loadsamples, numChannels |
		...
		// thisProcess.interpreter.executeFile(Platform.userAppSupportDir ++"/ixilang/"++project++"/synthdefs.scd");
		// was - > (Platform.userAppSupportDir ++"/ixilang/"++project++"/synthdefs.scd").loadPath;
...
*/
		...
		SynthDef(\morph, { arg out=0, freq=261.63, panFrom=0, panTo=0, amp=0.3, buf1, buf2, dur, morphtime=1, gate=1, t_trig, loop=1;
			...
			Out.ar(out,  Pan2.ar(IFFT.ar(chain), EnvGen.ar(Env.new([panFrom, panTo], [morphtime+0.3]), t_trig, doneAction:2)) * amp);
		}).add;
...
*/
		...
		SynthDef(\morph, { arg out=0, freq=261.63, panFrom=0, panTo=0, amp=0.3, buf1, buf2, dur, morphtime=1, gate=1, t_trig, loop=1;
			...
			chainB = FFT(LocalBuf(2048), inB);
			chain = PV_Morph(chainA, chainB, EnvGen.ar(Env.new([0, 1], [morphtime+0.1])) );
			Out.ar(out,  PanAz.ar(numChan, IFFT.ar(chain), EnvGen.ar(Env.new([panFrom, panTo], [morphtime+0.3]), t_trig, doneAction:2), orientation: 0) * amp);
		}).add;
		...
		doc.keyUpAction_({|doc, char, mod, unicode, keycode |
			...
			if((mod == 8388864).not, {
				...
				Routine.run {
					...
					Server.default.sync(cond);
					tempbuf.write((Platform.userAppSupportDir ++"/ixilang/"++project++"/livesamples/"++char++".aif"), "aiff", "int16");
